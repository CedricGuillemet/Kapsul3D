...
void CAffImage::PaintIt(CPoint point,int Factor)
{
    ...
    for (int i=0;i<32;i++)
    {
        for (int j=0;j<32;j++)
        {
            if ( ((point.x-j)>=0)&&((point.x-j)<1024)&&((1024-(point.y-i))>=0)&&((1024-(point.y-i))<1024) )
            {
                val=m_img->GetPix(point.x-j,1024-(point.y-i),((BYTE*)BmDest)+sizeof(BITMAPINFOHEADER));
                r=(val&255)+(((float)m_Zone->Brush[j][i]*m_Strengh)/256.0f)*Factor;
                if (r>255) r=255;
                if (r<0) r=0;
                val=r;//((char)(((255.0f/(float)daWaterLevel)*(float)r))<<16)+r;
                m_img->SetPix2(point.x-j,1024-(point.y-i),r,((BYTE*)BmDest)+sizeof(BITMAPINFOHEADER));
            }

            if ( ((point.x+j+1)>=0)&&((point.x+j+1)<1024)&&((1024-(point.y-i))>=0)&&((1024-(point.y-i))<1024) )
            {
                val=m_img->GetPix(point.x+j+1,1024-(point.y-i),((BYTE*)BmDest)+sizeof(BITMAPINFOHEADER));
                r=(val&255)+(((float)m_Zone->Brush[j][i]*m_Strengh)/256.0f)*Factor;
                if (r>255) r=255;
                if (r<0) r=0;
                val=((r<=daWaterLevel)?(((char)(((255.0f/(float)daWaterLevel)*(float)r))<<16)+r):((r<<16)+r));
                m_img->SetPix2(point.x+j+1,1024-(point.y-i),r,((BYTE*)BmDest)+sizeof(BITMAPINFOHEADER));
            }

            if ( ((point.x+j+1)>=0)&&((point.x+j+1)<1024)&&((1024-(point.y+i+1))>=0)&&((1024-(point.y+i+1))<1024) )
            {
                val=m_img->GetPix(point.x+j+1,1024-(point.y+i+1),((BYTE*)BmDest)+sizeof(BITMAPINFOHEADER));
                r=(val&255)+(((float)m_Zone->Brush[j][i]*m_Strengh)/256.0f)*Factor;
                if (r>255) r=255;
                if (r<0) r=0;
                val=((char)(((255.0f/(float)daWaterLevel)*(float)r))<<16)+r;
                m_img->SetPix2(point.x+j+1,1024-(point.y+i+1),r,((BYTE*)BmDest)+sizeof(BITMAPINFOHEADER));
            }

            if ( ((point.x-j)>=0)&&((point.x+j+1)<1024)&&((1024-(point.y+i+1))>=0)&&((1024-(point.y+i+1))<1024) )
            {
                val=m_img->GetPix(point.x-j,1024-(point.y+i+1),((BYTE*)BmDest)+sizeof(BITMAPINFOHEADER));
                r=(val&255)+(((float)m_Zone->Brush[j][i]*m_Strengh)/256.0f)*Factor;
                if (r>255) r=255;
                if (r<0) r=0;
                val=((char)(((255.0f/(float)daWaterLevel)*(float)r))<<16)+r;
                m_img->SetPix2(point.x-j,1024-(point.y+i+1),r,((BYTE*)BmDest)+sizeof(BITMAPINFOHEADER));
            }
