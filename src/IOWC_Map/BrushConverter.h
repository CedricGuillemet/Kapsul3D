...
extern int ALLOC_STEP;
extern DWORD dwTimer, dwStart;

extern int objcount, polycount;

#define TIMER_START() dwStart=GetTickCount();
#define TIMER_END() {dwTimer+=GetTickCount()-dwStart;}
...
public:
	...
	void	 Face_CalculateUV(SFACE *sFace);

	//Kapsul Desk Integration
	bool	 Desk_Import(CKScene *dadk,CBrushRaw *pBrush);
