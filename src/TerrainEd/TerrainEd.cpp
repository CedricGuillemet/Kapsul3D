...
CKTexture deftex(NULL,NULL);
CKMesh *TerrainObj=NULL;
...
void BuildObject(void)
{
	...
	int x,y;
	if (TerrainObj!=NULL)
		GScene->DelObject(TerrainObj);
//		delete TerrainObj;

	GScene->AddObject();
	TerrainObj=GScene->NextObject;
//	TerrainObj->Property1|=PROP_FORCETRI;
    ...
    for (y=0;y<128;y++)
	{
		for (x=0;x<128;x++)
		{
			...
			TerrainObj->pv[y*128+x].vz=lpBits[((y*av)*512)*3+(x*av)*3+2];
...
void PrecalcTerrain(void)
{
    ...
    int nColors = BiDest->biClrUsed ? BiDest->biClrUsed : 0x1FF & (1 << BiDest->biBitCount);
	if (nColors==256)
	{
		int av=m_Terra.GetWidth()/128;
		for (y=0;y<128;y++)
		{
			for (x=0;x<128;x++)
			{
				avt[y*128+x].x=x*16;
				avt[y*128+x].y=y*16;
				avt[y*128+x].z=lpBits[((y*av)*m_Terra.GetWidth())+(x*av)];
	...
	else
	{
		...
		for (y=0;y<128;y++)
		{
			for (x=0;x<128;x++)
			{
				avt[y*128+x].x=x*16;
				avt[y*128+x].y=y*16;
				avt[y*128+x].z=lpBits[((y*av)*m_Terra.GetWidth())*3+(x*av)*3+2];
