...
void CTerraTex::GenerateMap(MAPSINFO *lpMaps,CTerra *pTerrain,CTerraLight *pLight)
{
    ...
    for (y=0;y<pTerrain->GetHeight();y++)
    {
        for (x=0;x<pTerrain->GetWidth();x++)
        {
            ...
            xav=( ((x-1)>0)?(x-1):(x) );
            yav=( ((y-1)>0)?(y-1):(y) );
            xap=( ((x+1)<pTerrain->GetWidth())?(x+1):(x) );
            yap=( ((y+1)<pTerrain->GetHeight())?(y+1):(y) );

            float normalZ ;

            if ( (x>0)&&(x<(pTerrain->GetWidth()-1))&&(y>0)&&(y<(pTerrain->GetHeight()-1)) )
            {
            int tmp;
            /*
            BYTE *tmpb;
            ...
            */
			...
			heights[0] = lpBitsMaps2[tmp];
            tmp=(xav*3)+(y*pTerrain->iGridX*3);
            heights[1] = lpBitsMaps2[tmp];
			tmp=(x*3)+(yap*pTerrain->iGridX*3);
            heights[2] = lpBitsMaps2[tmp];
			tmp=(x*3)+(yav*pTerrain->iGridX*3);
            heights[3] = lpBitsMaps2[tmp];
*/

            tmp=(xap*3)+(y*pTerrain->GetWidth()*3);
			heights[0] = lpBitsMaps2[tmp];
            tmp=(xav*3)+(y*pTerrain->GetWidth()*3);
            heights[1] = lpBitsMaps2[tmp];
			tmp=(x*3)+(yap*pTerrain->GetWidth()*3);
            heights[2] = lpBitsMaps2[tmp];
			tmp=(x*3)+(yav*pTerrain->GetWidth()*3);
            heights[3] = lpBitsMaps2[tmp];
