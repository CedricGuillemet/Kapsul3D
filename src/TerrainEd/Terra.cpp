...
void CTerra::SetPix2(int x,int y,int val,BYTE *lpBits)
{
    ...
    if (val>WaterLevel)
    {
        ...
        lpBits[((x==0?0:x-1)*3)+((y==0?0:y-1)*GetWidth()*3)]=val;
        lpBits[((x==0?0:x-1)*3)+((y==0?0:y-1)*GetWidth()*3+1)]=val;
        lpBits[((x==0?0:x-1)*3)+((y==0?0:y-1)*GetWidth()*3+2)]=val;
    ...
    else
    {
        ...
        lpBits[((x==0?0:x-1)*3)+((y==0?0:y-1)*GetWidth()*3)]=tmp2;//(temp);//temp;
        lpBits[((x==0?0:x-1)*3)+(((y==0?0:y-1)*GetWidth()*3)+1)]=0;//(val)/2;
        lpBits[((x==0?0:x-1)*3)+(((y==0?0:y-1)*GetWidth()*3)+2)]=val;//(val)/2;
    }
...
#define GETPIX(x,y) lpBits[(((x==0?0:x-1)*3)+((y==0?0:y-1)*GetWidth()*3))+2]
...
void CTerra::FractalHalf(int iHalfValX,int iHalfValY,BYTE *lpBits)
{
    ...
    for (y=0;y<GetHeight();y+=iHalfValY)
    {
        for (x=0;x<GetWidth();x+=iHalfValX)
        {
/*
...
void CTerra::GenerateFractal()
{
    ...
    SetPix2((GetWidth()),0,(rand()&255),lpBits);

    SetPix2(0,(GetHeight()),(rand()&255),lpBits);

    SetPix2((GetWidth()),(GetHeight()),(rand()&255),lpBits);

    FractalHalf(GetWidth(),GetHeight(),lpBits);
...
double CTerra::FindHeightAt(double X,double Y)
{
    ...
    unsigned char h1=lpBits[dx*1+(dy*GetWidth()*1)];
    unsigned char h2=lpBits[(dx+1)*1+(dy*GetWidth()*1)];
    unsigned char h3=lpBits[dx*1+((dy+1)*GetWidth()*1)];
