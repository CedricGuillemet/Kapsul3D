...
//-----------------------------------------------------------------
// Standard mapping coordinate format using float
//-----------------------------------------------------------------

struct o_vert2f
{
	//data
	float u,v;	//Vertex coordinates

	//Some useful operator
	o_vert2f& operator == (o_vert2f &w) {u=w.u;v=w.v;return *this;}
	o_vert2f& operator = (float f) {u=v=f;return *this;}
	o_vert2f& operator += (o_vert2f &w) {u+=w.u;v+=w.v;return *this;}
	o_vert2f& operator -= (o_vert2f &w) {u-=w.u;v-=w.v;return *this;}
	o_vert2f& operator *= (o_vert2f &w) {u*=w.u;v*=w.v;return *this;}
	o_vert2f& operator /= (o_vert2f &w) {u/=w.u;v/=w.v;return *this;}
...
//-----------------------------------------------------------------
#define MAX_NUMBER_OF_MAPPING_LEVEL	4	//8 seems to be a good value
...
struct o_mesh
{
    ...
    o_vert3f*	verts;
	o_vert3f*	norms;
	o_vert2f*	mappingCoordinates[MAX_NUMBER_OF_MAPPING_LEVEL];
    o_poly*		polys;

//	o_triangle*	triangles;
    int numverts;				//size of verts
	int numvertsperframe;		//numberofvertexperframe
    int numpolys;				//size of polys array
    int numTextCoordinates;		//size of mappingCoordinates array
	int numofTextureLayer;		//The number of texture coordinate layer - up to MAX_NUMBER_OF_MAPPING_LEVEL
	int numFrames;				//Number of frame
	o_mesh *next;
	o_mesh *prev;
};
...
//			delete (tm->triangles);
			for (i=0;i<MAX_NUMBER_OF_MAPPING_LEVEL;i++)
				{
				delete (tm->mappingCoordinates[i]);
				}
			delete (tm->polys);
			if (!tm->next) break;
			tm = tm->next;
