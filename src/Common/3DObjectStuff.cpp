...
int i=0,j=0,k=0;
	...
	if (TheKapsulScene != NULL)	//Is there a valid Kapsul scene ?
		...
		for (k=0;k<The3DObject->nummeshes;k++)
			...
			// Allocate and fill up texture coordinate buffer
			if (pOMesh->numTextCoordinates!=0)
				{
				pCKMesh->AllocateUV(0,pOMesh->numTextCoordinates);
				for (i=0;i<pOMesh->numTextCoordinates; i++)
					{
					pCKMesh->nbTexCanals = pOMesh->numofTextureLayer;

					for (j=0; j<pOMesh->numofTextureLayer;j++)
						{
						pCKMesh->puv[j][i].u = pOMesh->mappingCoordinates[j][i].u;
						pCKMesh->puv[j][i].v = pOMesh->mappingCoordinates[j][i].v;
						}
					}
				}
				...
				if (pOPolys->NumofVertex<3 )	//Triangle is the first primitive we are handling in Kapsul editor
					{
						...
						if (pOPolys->NumofVertex==1)
						{
							...
							pCKMesh->pl[i].Tex[0][0] = pOPolys->verts[0];
							pCKMesh->pl[i].nbvert[1] = pOPolys->verts[0];
							pCKMesh->pl[i].Tex[0][1] = pOPolys->verts[0];
							pCKMesh->pl[i].nbvert[2] = pOPolys->verts[0];
							pCKMesh->pl[i].Tex[0][2] = pOPolys->verts[0];
						}
						else
						{
							pCKMesh->pl[i].nbvert[0] = pOPolys->verts[0];
							...
							pCKMesh->pl[i].Tex[0][1] = pOPolys->verts[1];
							pCKMesh->pl[i].nbvert[2] = pOPolys->verts[0];
							pCKMesh->pl[i].Tex[0][2] = pOPolys->verts[0];
						}
					}
					...
					for (j=0;j<pCKMesh->pl[i].nb2vert;j++)	//Looping on the number of vertex for this polygon
						...
						switch(pCKMesh->nbTexCanals)	//Depending on the number of texture coordinates channels
							...
							case 1:
								...
								//Texture coordinates
	//							pOMesh->mappingCoordinates[0][j]
								pCKMesh->pl[i].Tex[0][j] = pOPolys->verts[j];	//For the moment, need to be modify
							break;
							...
							case 2:
								...
								//Texture coordinates
								pCKMesh->pl[i].Tex[0][j] = pOPolys->verts[j];	//For the moment, need to be modify
								pCKMesh->pl[i].Tex[1][j] = pOPolys->verts[j];	//For the moment, need to be modify
							break;
							...
							case 3:
								...
								pCKMesh->pl[i].Tex[0][j] = pOPolys->verts[j];	//For the moment, need to be modify
								pCKMesh->pl[i].Tex[1][j] = pOPolys->verts[j];	//For the moment, need to be modify
								pCKMesh->pl[i].Tex[2][j] = pOPolys->verts[j];	//For the moment, need to be modify
							break;
							...
							case 4:
								...
								pCKMesh->pl[i].Tex[1][j] = pOPolys->verts[j];	//For the moment, need to be modify
								pCKMesh->pl[i].Tex[2][j] = pOPolys->verts[j];	//For the moment, need to be modify
								pCKMesh->pl[i].Tex[3][j] = pOPolys->verts[j];	//For the moment, need to be modify
							break;
							}
						}
					}
