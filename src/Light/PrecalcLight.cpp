...
void CPrecalcLight::OnCalculate() 
{
	...
	LightMapSize=1024;//ListLightMapSize[m_Size.GetCurSel()];
	LumelSize=4;//ListLumelSize[m_Density.GetCurSel()];
	LIGHTMAP_SIZE=1024;//LightMapSize;
	...
	// precalc spots
	//for (daLight=GScene->FirstLight;daLight!=NULL;daLight=daLight->Next)
	for (CKEntity *daEnt=GScene->FirstEntity;daEnt!=NULL;daEnt=daEnt->Next)
	{
		if (strcmp(daEnt->EntType,"Light")==0)
		{
			daLight=(CLight*)daEnt->CompilData;
			daLight->PrecalcSpot();
		}
	}
	...
	for (daMesh=GScene->FirstMesh;daMesh!=NULL;daMesh=daMesh->Next)
	{
		for (i=0;i<daMesh->nb2poly;i++)
		{
			...
			memset(txsx[TotalPoly].lpBits,0x70,sx*sy*3);
...
//			for (daLight=GScene->FirstLight;daLight!=NULL;daLight=daLight->Next)
	...
	GScene->InitAllTag(-1,PLUGIN_MESH);
	...
	packer.AddToScene(GScene,true,1);
