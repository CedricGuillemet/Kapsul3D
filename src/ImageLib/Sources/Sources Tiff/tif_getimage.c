/* $Header: C:/cvs/Repository/Kapsul3D/src/ImageLib/Sources/Sources\040Tiff/tif_getimage.c,v 1.1 2002/03/29 05:06:26 khurley Exp $ */
...
setupMap(TIFFRGBAImage* img)
{
	...
	for (x = 0; x <= range; x++)
	    img->Map[x] = (unsigned char)(((range - x) * 255) / range);
    } else {
	for (x = 0; x <= range; x++)
	    img->Map[x] = (unsigned char)((x * 255) / range);
    }
