/* $Header: C:/cvs/Repository/Kapsul3D/src/ImageLib/Sources/Sources\040Tiff/tif_dirread.c,v 1.1 2002/03/29 05:06:25 khurley Exp $ */
...
TIFFReadDirectory(TIFF* tif)
{
	...
	if (!isMapped(tif)) {
		...
		if (dir == NULL)
			return (0);
		if (!ReadOK(tif, dir, dircount*(int)sizeof (TIFFDirEntry))) {
		...
		if (off + (int)sizeof (uint16) > tif->tif_size) {
			TIFFError(tif->tif_name,
			    "Can not read TIFF directory count");
			return (0);
		} else
		...
		if (dir == NULL)
			return (0);
		if (off + dircount*(int)sizeof (TIFFDirEntry) > tif->tif_size) {
			TIFFError(tif->tif_name, "Can not read TIFF directory");
			goto bad;
		} else
		...
		if (off + (int)sizeof (uint32) <= tif->tif_size)
...
EstimateStripByteCounts(TIFF* tif, TIFFDirEntry* dir, uint16 dircount)
{
	...
	if (td->td_compression != COMPRESSION_NONE) {
		 ...
		 */ 
		i--;
		if (td->td_stripoffset[i] + td->td_stripbytecount[i] > (unsigned int)filesize)
...
TIFFFetchByteArray(TIFF* tif, TIFFDirEntry* dir, uint16* v)
{
	if (dir->tdir_count <= 4) {
		...
		if (tif->tif_header.tiff_magic == TIFF_BIGENDIAN) {
			...
			case 4: v[3] = (unsigned short)dir->tdir_offset & 0xff;
			case 3: v[2] = (unsigned short)(dir->tdir_offset >> 8) & 0xff;
			case 2: v[1] = (unsigned short)(dir->tdir_offset >> 16) & 0xff;
			case 1: v[0] = (unsigned short)dir->tdir_offset >> 24;
			}
		} else {
			switch (dir->tdir_count) {
			case 4: v[3] = (unsigned short)dir->tdir_offset >> 24;
			case 3: v[2] = (unsigned short)(dir->tdir_offset >> 16) & 0xff;
			case 2: v[1] = (unsigned short)(dir->tdir_offset >> 8) & 0xff;
			case 1: v[0] = (unsigned short)dir->tdir_offset & 0xff;
			}
		}
		return (1);
	} else
...
TIFFFetchShortArray(TIFF* tif, TIFFDirEntry* dir, uint16* v)
{
	if (dir->tdir_count <= 2) {
		if (tif->tif_header.tiff_magic == TIFF_BIGENDIAN) {
			switch (dir->tdir_count) {
			case 2: v[1] = (unsigned short)dir->tdir_offset & 0xffff;
			case 1: v[0] = (unsigned short)dir->tdir_offset >> 16;
			}
		} else {
			switch (dir->tdir_count) {
			case 2: v[1] = (unsigned short)dir->tdir_offset >> 16;
			case 1: v[0] = (unsigned short)dir->tdir_offset & 0xffff;
			}
		}
		return (1);
	} else
