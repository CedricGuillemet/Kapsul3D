...
BOOL CCoolDialogBar::Create(CWnd* pParentWnd, CTScrollDlg *pDialog, CString &pTitle, UINT nID, DWORD dwStyle) 
{
...
CSize CCoolDialogBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
	...
	if(IsFloating())
        ...
        return CSize(200-26, nVertDockBarHeight);
}
...
CSize CCoolDialogBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
	...
	if (dwMode & LM_LENGTHY)
        ...
        return CSize(200-26, m_sizeFloat.cy);
}
...
void CCoolDialogBar::OnWindowPosChanged(WINDOWPOS FAR* lpwndpos) 
{
    ...
    if (m_bInRecalcNC) 
	{
		...
		m_cDialog->SetWindowPos(NULL,0,0,200-18,rc.bottom,SWP_NOMOVE   |SWP_NOZORDER);
...
//	cx=200-26;
		...
		if (GCurPlug!=NULL)
		{
			GCurPlug->ReSize(200-18,rc.bottom-rc.top-10);
		}
...
void CCoolDialogBar::OnSizing( UINT nSide, LPRECT lpRect )
{
