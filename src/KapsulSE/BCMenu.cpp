...
void BCMenu::DrawItem_WinXP (LPDRAWITEMSTRUCT lpDIS)
{
	...
	if(state & MF_SEPARATOR){
		...
		COLORREF crText = GetSysColor(COLOR_MENUTEXT);
		COLORREF crSelect2;
		crSelect2= GetSysColor(COLOR_HIGHLIGHT);
		COLORREF crSelectFill;
		if(!IsWinXPLuna())crSelectFill=LightenColor(crSelect2,0.85);
		else crSelectFill=LightenColor(crSelect2,0.7);
		CBrush m_brSelect;
		CPen m_penBack;
		int x0,y0,dx,dy;
		int nIconNormal=-1,xoffset=-1,global_offset=-1;
		...
		if(state&ODS_SELECTED){ // draw the down edges
			...
			pDC->Draw3dRect (rect,crSelect2,crSelect2);
		...
		if(checkflag||standardflag||selectedflag||disableflag){
			...
			if(checkflag && checkmaps){
				...
				if(!selectedflag){
					...
					if(state&ODS_SELECTED)pDC->Draw3dRect (rect,crSelect2,crSelect2);
					bitmapstandard.DeleteObject();
				}
			}
			else if(selectedflag){
				...
				CPoint ptImage(rect.left+dx,rect.top+dy);
				if(state&ODS_CHECKED){
					pDC->Draw3dRect(rect2,crSelect2,crSelect2);
				...
				if(state&ODS_CHECKED){
					CBrush brushin;
					brushin.CreateSolidBrush(LightenColor(crSelect2,0.85));
		...
		if(nIconNormal<0 && state&ODS_CHECKED && !checkflag){
			...
			if(state&ODS_CHECKED || info.hbmpUnchecked) {
				...
				info.hbmpUnchecked),(unsigned long)crSelect2);
			}
		}
...
BOOL BCMenu::LoadMenu(LPCTSTR lpszResourceName)
{
	...
	do{
		...
		// BIG HACK for localization
			char ansiname[256];
			WideCharToMultiByte( CP_ACP, 0, szCaption, -1,ansiname, 256, NULL, NULL );
			int LocID=loc.MenuFindPopupID(ansiname);
			CString cvname=loc.GetString(LocID);
			if (!cvname.IsEmpty())
			{
				WCHAR wszLocName[256];          
				MultiByteToWideChar( CP_ACP, 0, cvname.GetBuffer(0),cvname.GetLength()+1, wszLocName,sizeof(wszLocName)/sizeof(wszLocName[0]) );
				m_Stack[m_Stack.GetUpperBound()]->AppendODMenuW(wszLocName,uFlags,
