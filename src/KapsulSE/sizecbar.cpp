...
CSize CSizingControlBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
    if (bStretch) // the bar is stretched (is not the child of a dockbar)
        if (bHorz)
            return CSize(32767, ((m_szFixedHeight==0)?m_szHorz.cy:m_szFixedHeight));
        else
            return CSize(((m_szFixedWidth==0)?m_szVert.cx:m_szFixedWidth), 32767);
    ...
    if (bHorz)
        return CSize(((m_szFixedWidth==0)?max(m_szMinHorz.cx, m_szHorz.cx):m_szFixedWidth),
                     ((m_szFixedHeight==0)?max(m_szMinHorz.cy, m_szHorz.cy):m_szFixedHeight) );

    return CSize(((m_szFixedWidth==0)?max(m_szMinVert.cx, m_szVert.cx):m_szFixedWidth),
			((m_szFixedHeight==0)?max(m_szMinVert.cy, m_szVert.cy):m_szFixedHeight) );
}
...
CSize CSizingControlBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
    ...
    m_szFloat.cx = ((m_szFixedWidth==0)?max(m_szFloat.cx, m_szMinFloat.cx):m_szFixedWidth);
    m_szFloat.cy = ((m_szFixedHeight==0)?max(m_szFloat.cy, m_szMinFloat.cy):m_szFixedHeight);
