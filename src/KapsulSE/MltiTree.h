...
CImageList* m_pDragImage;
	...
	bool        m_bLDragging;
	bool		CanDropFile;
	UINT		m_idExpandTimer;
	UINT		m_idScrollTimer;
	UINT		m_timerticks;
...
public:
	...
	void CanBeTargetForCKViewEdit(void);
...
protected:
	...
	void DropEnd(UINT nFlags, CPoint point);
	void DuringDrag(UINT nFlags, CPoint point);
// Implementation
public:
	HTREEITEM hContextItem;
	virtual ~CMultiTree();
	int GetItemId(HTREEITEM item);
	LPARAM GetItemParam(HTREEITEM item);
	void SetItemParam(HTREEITEM item,LPARAM lpr);
	bool IsItemSelected(HTREEITEM item);
	bool IsItemChecked(HTREEITEM hItem);
	bool SetItemChecked(HTREEITEM hItem,bool Checked);
