...
//	m_bStartup = !pApp->GetProfileInt(szSection, szIntStartup, 0);
	...
	Tips.EraseList();
	Tips.ReadXmlStringFile("Tips.xml");
	if (Tips.GetStringCount()==0)
	{
		m_pStream=false;	
		m_strTip=loc.GetString(194);
		return;
	} 
	m_pStream=true;
}
...
void CTipDlg::GetNextTipString(CString& strNext)
{
	...
	strNext=Tips.GetString(iFilePos+1);
	iFilePos++;

	if (iFilePos>=(Tips.GetStringCount()))
		iFilePos=0;
}
...
void CTipDlg::OnOK()
{
	...
	CWinApp* pApp = AfxGetApp();
	pApp->WriteProfileInt(szSection, szIntFilePos, iFilePos );
...
BOOL CTipDlg::OnInitDialog()
{
	...
	SetWindowText(loc.GetString(198));
	
	((CButton*)GetDlgItem(IDC_STARTUP))->SetWindowText(loc.GetString(199));
	((CButton*)GetDlgItem(IDC_NEXTTIP))->SetWindowText(loc.GetString(200));
