...
bool CLoadASE::ImportASE(Desk *dk, char *strFileName)
{
...
void CLoadASE::ReadAseFile(Desk *dk)
{
	...
	int i = 0;	
	int numOfObjects,numOfMaterials ;
...
void CLoadASE::ReadObjectInfo(Object *pObject, int desiredObject)
{
	...
	while (!feof(m_FilePointer))
	{
		...
		if (!strcmp(strWord, NUM_VERTEX))
		{
			...
			int nb2vert;
			fscanf(m_FilePointer, "%d", &nb2vert);
			pObject->AllocatePoly(0,nb2vert);
			pObject->AllocateUV(0,nb2vert);
		...
		else if (!strcmp(strWord, NUM_FACES))
		{
			...
			int nb2faces;
			fscanf(m_FilePointer, "%d", &nb2faces);

			// Allocate enough memory to hold the faces
			//pObject->pFaces = new tFace [pObject->numOfFaces];
			pObject->AllocatePoly(nb2faces,0);
		}
...
void CLoadASE::ReadObjectData(Object *pObject, int desiredObject)
{
	...
	GetData(pObject, MATERIAL_ID, desiredObject);

	// Load the vertices for this object
	GetData(pObject, VERTEX,		 desiredObject);

	// Load the texture coordinates for this object
	GetData(pObject, TVERTEX,	 desiredObject);

	// Load the vertex faces list for this object
	GetData(pObject, FACE,		 desiredObject);

	// Load the texture face list for this object
	GetData(pObject, TFACE,		 desiredObject);

	// Load the texture for this object
	GetData(pObject, TEXTURE,	 desiredObject);

	// Load the U tile for this object
	GetData(pObject, UTILE,		 desiredObject);

	// Load the V tile for this object
	GetData(pObject, VTILE,		 desiredObject);
}
...
void CLoadASE::GetData(Object *pObject, char *strDesiredData, int desiredObject)
{
	char strWord[255] = {0};
	...
	while(!feof(m_FilePointer))
	{
		...
		else if(!strcmp(strWord, TVERTEX))
		{
			...
			if(!strcmp(strDesiredData, TVERTEX)) 
			{
				...
				ReadTextureVertex(pObject, NULL);//pModel->pMaterials[pObject->materialID]);
			}
		}
...
void CLoadASE::ReadVertex(Object *pObject)
{
	...
	fscanf(m_FilePointer, "%f %f %f", &pObject->pv[index].vx, 
									  &pObject->pv[index].vy,
									  &pObject->pv[index].vz);
	...
	pObject->pv[index].vy = -pObject->pv[index].vy;
}
...
void CLoadASE::ReadTextureVertex(Object *pObject, tMaterialInfo *texture)
{
	...
	fscanf(m_FilePointer, "%f %f", &(pObject->puv[0][index].u), &(pObject->puv[0][index].v));
	...
	/*
	pObject->puv[0][index].u *= texture.uTile;
	pObject->puv[0][index].v *= texture.vTile;
*/
...
void CLoadASE::ReadFace(Object *pObject)
{
	...
	pObject->pl[index].Allocate(pObject,1,3);
	fscanf(m_FilePointer, "\tA:\t%d B:\t%d C:\t%d", &pObject->pl[index].nbvert[2], 
													&pObject->pl[index].nbvert[1], 
													&pObject->pl[index].nbvert[0]); 

	pObject->pl[index].Tex[0][2]=pObject->pl[index].nbvert[2];
	pObject->pl[index].Tex[0][1]=pObject->pl[index].nbvert[1];
	pObject->pl[index].Tex[0][0]=pObject->pl[index].nbvert[0];
...
void CLoadASE::ReadTextureFace(Object *pObject)
{
	...
	fscanf(m_FilePointer, "%d %d %d", &pObject->pl[index].Tex[0][0], 
									  &pObject->pl[index].Tex[0][1], 
									  &pObject->pl[index].Tex[0][2]);
}
