...
public:
    ...
    inline bool Intersect(const Vertex &start, const Vertex &end, double *time)
    {
        ...
        float const *bmin = &pta.x;
        float const *bmax = &ptb.x;
    ...
    {
        if ( sqrt( (pta.x-Center.x)*(pta.x-Center.x) +
            (pta.y-Center.y)*(pta.y-Center.y) +
            (pta.z-Center.z)*(pta.z-Center.z) ) <=Radius) return true;

        if ( sqrt( (ptb.x-Center.x)*(ptb.x-Center.x) +
            (pta.y-Center.y)*(pta.y-Center.y) +
            (pta.z-Center.z)*(pta.z-Center.z) ) <=Radius) return true;

        if ( sqrt( (pta.x-Center.x)*(pta.x-Center.x) +
            (ptb.y-Center.y)*(ptb.y-Center.y) +
            (pta.z-Center.z)*(pta.z-Center.z) ) <=Radius) return true;
        ...
        if ( sqrt( (pta.x-Center.x)*(pta.x-Center.x) +
            (pta.y-Center.y)*(pta.y-Center.y) +
            (ptb.z-Center.z)*(ptb.z-Center.z) ) <=Radius) return true;
