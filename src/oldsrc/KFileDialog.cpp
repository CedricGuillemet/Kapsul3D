...
BEGIN_MESSAGE_MAP(CKFileDialog, CDialog)
	...
	ON_CBN_SELCHANGE(IDC_EXTENSIONS, OnSelchangeExts)
	ON_WM_SIZE()
	ON_WM_CLOSE()
END_MESSAGE_MAP()
...
void CKFileDialog::OnBnClickedBack()
{
	if (BackList.size()>=2)
	{
...
void CKFileDialog::OnBnClickedUp()
{
	m_ListFile.m_strImageDir.SetAt(m_ListFile.m_strImageDir.GetLength()-1,0);
	if (m_ListFile.m_strImageDir.ReverseFind('\\')>m_ListFile.m_strImageDir.Find('\\'))
	{
...
void CKFileDialog::SetTypes(CString Types)
{
	int i;
	bool extval=false;
...
void CKFileDialog::SetInitialDir(CString InitialDir)
{
	m_ListFile.m_strImageDir=InitialDir;
}

void CKFileDialog::IsForOpen()
{
	IsForSaving=false;
	BtOkStr="Open";
}

void CKFileDialog::IsForSave()
{
	IsForSaving=true;
	BtOkStr="Save";
}

void CKFileDialog::OnSelchangeExts()
{
...
void CKFileDialog::OnBnClickedCancel()
{
	OnClose();
	CDialog::OnCancel();
}

void CKFileDialog::OnBnClickedOk()
{
	OnClose();
	CString filth;
