...
CFont *CCrystalTextView::GetFont(BOOL bItalic /*= FALSE*/, BOOL bBold /*= FALSE*/)
{
	...
	if (m_apFonts[nIndex] == NULL)
	{
		...
		m_lfBaseFont.lfWeight=10;
		m_lfBaseFont.lfWidth=2;
...
void CCrystalTextView::OnBeginPrinting(CDC *pdc, CPrintInfo *pInfo)
{
	...
	lf.lfHeight = MulDiv(lf.lfHeight, pdc->GetDeviceCaps(LOGPIXELSY), pDisplayDC->GetDeviceCaps(LOGPIXELSY) * 4);
	lf.lfWidth = MulDiv(lf.lfWidth, pdc->GetDeviceCaps(LOGPIXELSX), pDisplayDC->GetDeviceCaps(LOGPIXELSX) * 4);
...
int CCrystalTextView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	lstrcpy(m_lfBaseFont.lfFaceName, _T("DEFAULT"));
	m_lfBaseFont.lfHeight = 10;
	...
	ASSERT(m_hAccel == NULL);
	m_hAccel = ::LoadAccelerators(GetResourceHandle(), MAKEINTRESOURCE(IDR_KAPSULSCRIPT));
	ASSERT(m_hAccel != NULL);
	return 0;
}
