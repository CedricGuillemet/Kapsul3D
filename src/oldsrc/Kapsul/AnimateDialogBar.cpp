...
BOOL CAnimateDialogBar::Create(CWnd* pParentWnd, CAnimate *pDialog, CString &pTitle, UINT nID, DWORD dwStyle) 
{
...
CSize CAnimateDialogBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
	...
	if(IsFloating())
        ...
        return CSize(nHorzDockBarWidth,58);
}
...
CSize CAnimateDialogBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
	...
	if (dwMode & LM_LENGTHY)
        return CSize(
            m_sizeFloat.cx = max(m_sizeMin.cx, nLength),50);
    else
    */
...
void CAnimateDialogBar::OnWindowPosChanged(WINDOWPOS FAR* lpwndpos) 
{
    ...
    if (m_bInRecalcNC) 
	{
		...
		m_cDialog->SetWindowPos(NULL,0,0,rc.right,58,SWP_NOMOVE   |SWP_NOZORDER);
