...
CAnimateDialogBar::CAnimateDialogBar() : 
...
BEGIN_MESSAGE_MAP(CAnimateDialogBar, CControlBar)
    //{{AFX_MSG_MAP(CAnimateDialogBar)
    ON_WM_LBUTTONUP()
    ON_WM_MOUSEMOVE()
    ON_WM_SETCURSOR()
...
/////////////////////////////////////////////////////////////////////////////
// CAnimateDialogBar message handlers

void CAnimateDialogBar::OnUpdateCmdUI(class CFrameWnd *pTarget, int bDisableIfNoHndler)
{
    UpdateDialogControls(pTarget, bDisableIfNoHndler);
}

BOOL CAnimateDialogBar::Create(CWnd* pParentWnd, CAnimOutput *pDialog, CString &pTitle, UINT nID, DWORD dwStyle) 
{
	...
	//GetClientRect(&rc);
//	cs.Create(CRect(rc.right,0,rc.right+8,rc.bottom-10),m_cDialog,0);
    return TRUE;
}

CSize CAnimateDialogBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
    CRect rc;
	...
	if(IsFloating())
        ...
        return CSize( m_sizeHorz.cx,nVertDockBarHeight);
    else
        return CSize(nHorzDockBarWidth,m_sizeHorz.cy);
}

CSize CAnimateDialogBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
	...
	if (dwMode & LM_LENGTHY)
        return CSize(m_sizeFloat.cx = max(m_sizeMin.cx, nLength),50);
    else
    
        return CSize(m_sizeFloat.cx = max(m_sizeMin.cx, nLength),58);
		*/
	if (dwMode & LM_LENGTHY)
        return CSize(
            m_sizeFloat.cx = max(m_sizeMin.cx, nLength),m_sizeFloat.cy = max(m_sizeMin.cy, nLength));
    else
           return CSize(m_sizeFloat.cx = max(m_sizeMin.cx, nLength),m_sizeFloat.cy);
}

void CAnimateDialogBar::OnWindowPosChanged(WINDOWPOS FAR* lpwndpos) 
{
    ...
    if (m_bInRecalcNC) 
	{
		...
		m_cDialog->GetClientRect(rc);
		rc.bottom=rc2.bottom;
		//m_cDialog->MoveWindow(rc,TRUE);
		m_cDialog->SetWindowPos(NULL,0,0,rc.right,rc.bottom,SWP_NOMOVE   |SWP_NOZORDER);
...
BOOL CAnimateDialogBar::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
// Mouse Handling
//
void CAnimateDialogBar::OnLButtonUp(UINT nFlags, CPoint point) 
{
    if (!m_bTracking)
...
void CAnimateDialogBar::OnMouseMove(UINT nFlags, CPoint point) 
{
...
void CAnimateDialogBar::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR* lpncsp) 
{
...
void CAnimateDialogBar::OnNcPaint() 
{
    EraseNonClient();
    ...
    m_cDialog->Invalidate(FALSE);
}

void CAnimateDialogBar::OnNcLButtonDown(UINT nHitTest, CPoint point) 
{
...
UINT CAnimateDialogBar::OnNcHitTest(CPoint point) 
{
    if (IsFloating())
...
void CAnimateDialogBar::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void CAnimateDialogBar::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
...
void CAnimateDialogBar::StartTracking()
{
    SetCapture();
...
void CAnimateDialogBar::OnCaptureChanged(CWnd *pWnd) 
{
...
void CAnimateDialogBar::StopTracking(BOOL bAccept)
{
    OnInvertTracker(m_rectTracker);
...
void CAnimateDialogBar::OnInvertTracker(const CRect& rect)
{
    ASSERT_VALID(this);
...
BOOL CAnimateDialogBar::IsHorz() const
{
    return (m_nDockBarID == AFX_IDW_DOCKBAR_TOP ||
        m_nDockBarID == AFX_IDW_DOCKBAR_BOTTOM);
}

CPoint& CAnimateDialogBar::ClientToWnd(CPoint& point)
{
...
void CAnimateDialogBar::DrawGripper(CDC & dc)
{
...
void CAnimateDialogBar::OnNcLButtonDblClk(UINT nHitTest, CPoint point) 
{
...
void CAnimateDialogBar::OnSizing( UINT nSide, LPRECT lpRect )
{
	//lpRect->right=lpRect->left+200;

	CControlBar::OnSizing(nSide,lpRect);

}
extern GenericLib *gen;

void CAnimateDialogBar::OnSize( UINT nType, int cx, int cy )
{
	cx=200-26;
    RECT rc;
    ...
    if (gen!=NULL)
    {
        gen->Process(KM_MOVEWINDOW,cx,cy-10);
    }
