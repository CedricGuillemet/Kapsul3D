...
CDesksShowDialogBar::CDesksShowDialogBar() : 
...
BEGIN_MESSAGE_MAP(CDesksShowDialogBar, CControlBar)
    //{{AFX_MSG_MAP(CDesksShowDialogBar)
    ON_WM_LBUTTONUP()
    ON_WM_MOUSEMOVE()
    ON_WM_SETCURSOR()
...
/////////////////////////////////////////////////////////////////////////////
// CDesksShowDialogBar message handlers

void CDesksShowDialogBar::OnUpdateCmdUI(class CFrameWnd *pTarget, int bDisableIfNoHndler)
{
    UpdateDialogControls(pTarget, bDisableIfNoHndler);
}

BOOL CDesksShowDialogBar::Create(CWnd* pParentWnd, CTVTestDlg *pDialog, CString &pTitle, UINT nID, DWORD dwStyle) 
{
    ...
    CString wndclass = AfxRegisterWndClass(CS_DBLCLKS, LoadCursor(NULL, IDC_ARROW),
        m_brushBkgd, 0);
    if (!CWnd::Create(wndclass, pTitle, dwStyle, CRect(0,0,150,500),
        pParentWnd, 184784))
        return FALSE;

	// create the child dialog
	
	m_cDialog = (CTVTestDlg*)pDialog;
...
CSize CDesksShowDialogBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
    CRect rc;
...
CSize CDesksShowDialogBar::CalcDynamicLayout(int nLength, DWORD dwMode)
{
...
void CDesksShowDialogBar::OnWindowPosChanged(WINDOWPOS FAR* lpwndpos) 
{
...
BOOL CDesksShowDialogBar::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
...
// Mouse Handling
//
void CDesksShowDialogBar::OnLButtonUp(UINT nFlags, CPoint point) 
{
    if (!m_bTracking)
...
void CDesksShowDialogBar::OnMouseMove(UINT nFlags, CPoint point) 
{
...
void CDesksShowDialogBar::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR* lpncsp) 
{
...
void CDesksShowDialogBar::OnNcPaint() 
{
    EraseNonClient();
...
void CDesksShowDialogBar::OnNcLButtonDown(UINT nHitTest, CPoint point) 
{
...
UINT CDesksShowDialogBar::OnNcHitTest(CPoint point) 
{
    if (IsFloating())
...
void CDesksShowDialogBar::OnLButtonDown(UINT nFlags, CPoint point) 
{
...
void CDesksShowDialogBar::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
...
void CDesksShowDialogBar::StartTracking()
{
    SetCapture();
...
void CDesksShowDialogBar::OnCaptureChanged(CWnd *pWnd) 
{
...
void CDesksShowDialogBar::StopTracking(BOOL bAccept)
{
    OnInvertTracker(m_rectTracker);
...
void CDesksShowDialogBar::OnInvertTracker(const CRect& rect)
{
    ASSERT_VALID(this);
...
BOOL CDesksShowDialogBar::IsHorz() const
{
    return (m_nDockBarID == AFX_IDW_DOCKBAR_TOP ||
        m_nDockBarID == AFX_IDW_DOCKBAR_BOTTOM);
}

CPoint& CDesksShowDialogBar::ClientToWnd(CPoint& point)
{
...
void CDesksShowDialogBar::DrawGripper(CDC & dc)
{
...
void CDesksShowDialogBar::OnNcLButtonDblClk(UINT nHitTest, CPoint point) 
{
...
void CDesksShowDialogBar::OnSizing( UINT nSide, LPRECT lpRect )
{
...
void CDesksShowDialogBar::OnSize( UINT nType, int cx, int cy )
{
	cx=200-26;
    RECT rc;
