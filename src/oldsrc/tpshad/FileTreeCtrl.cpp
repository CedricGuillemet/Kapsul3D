...
*/
    if (GetZipName(daff.GetBuffer(1),ZipName,ZipRep))
  {
      // archive
      ...
      int RepAv=0;
      char *fname=pak.GetFolder(ZipRep,RepAv++);
...
*/
  ...
  CString tmpx=sPath;//.GetBuffer(1);
  if (GetZipName(tmpx.GetBuffer(1),ZipName,ZipRep))
  {
      // archive
      ...
      while (fname!=NULL)
      {
        ...
        {
            int fnptr=strlen(fname);
            while (fname[fnptr]!='\\') fnptr--;
            fnptr++;
            fname+=fnptr;
        }
        //DirectoryPaths.Add(sPath2);

        InsertFileItem(fname, sPath2.GetBuffer(1), hParent);

          fname=pak.GetFolder(ZipRep,RepAv++);
      }
      /*
        if (bUseSetRedraw)
            SetRedraw(TRUE);
